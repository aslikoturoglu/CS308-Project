  ...
  db.query(lookupSql, [orderItemId, userId], (err, rows) => {
    ...
    db.query(existingSql, [orderItemId], (existingErr, existingRows) => {
      ...
      db.query(insertSql, [orderItemId, userId, reason], (insertErr, result) => {
        ...
        db.query(
          "UPDATE deliveries SET delivery_status = 'refund_waiting' WHERE order_item_id = ?",
          [orderItemId],
          (deliveryErr) => {
            if (deliveryErr) {
              console.error("Delivery status update failed:", deliveryErr);
              return res.status(500).json({ error: "Delivery status update failed" });
            }
            db.query(
              "UPDATE orders SET status = 'refund_waiting' WHERE order_id = ?",
              [row.order_id],
              (orderErr) => {
                if (orderErr) {
